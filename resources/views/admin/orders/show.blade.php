        ...
        <div class="col-lg-8">
            ...
            <div class="card mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <div>
                        <span class="badge bg-{{ $order->payment_badge }} me-2">Payment: {{ ucfirst($order->payment_status) }}</span>
                        ...
                        <div id="credentials-fields" style="display: none;">
                            <div class="row">
                                ...
                                <div class="col-12 mb-3">
                                    ...
                                    <input type="url" name="portal_url" class="form-control" placeholder="http://your-portal-url.com:port">
                                </div>
                            </div>
                        </div>
        ...
        <div class="col-lg-4">
            ...
            @if($order->user)
                ...
                @if($order->user->referred_by)
                    @php
                        $referrer = \App\Models\User::find($order->user->referred_by);
                        $commission = \App\Models\Commission::where('order_id', $order->id)->first();
                    @endphp
                    @if($referrer)
                        <div class="card mb-4 border-info">
                            <div class="card-header bg-info bg-opacity-10 text-info fw-bold">Affiliate / Referral Info</div>
                            <div class="card-body">
                                <p class="text-muted small mb-1">Referred By</p>
                                <div class="fw-medium mb-3">{{ $referrer->name }} ({{ $referrer->email }})</div>
                                
                                @if($commission)
                                    <p class="text-muted small mb-1">Commission Earned</p>
                                    <div class="fw-bold text-success fs-5 mb-2">${{ number_format($commission->commission_amount, 2) }}</div>
                                    <span class="badge bg-{{ $commission->status === 'paid' ? 'success' : ($commission->status === 'approved' ? 'primary' : 'warning') }}">
                                        Status: {{ ucfirst($commission->status) }}
                                    </span>
                                @else
                                    <p class="text-muted small mb-0">No commission generated for this order.</p>
                                @endif
                                
                                <a href="{{ route('admin.users.show', $referrer) }}" class="btn btn-outline-info btn-sm w-100 mt-3">
