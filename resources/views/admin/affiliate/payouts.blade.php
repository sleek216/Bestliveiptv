                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="pe-4 text-end">
                            ...
                            @elseif($payout->status === 'processing')
                                <button type="button" class="btn btn-sm btn-success text-white" data-bs-toggle="modal" data-bs-target="#completePayoutModal{{ $payout->id }}">
                                    Mark Completed
                                </button>
                                
                                <!-- Complete Payout Modal -->
                                <div class="modal fade text-start" id="completePayoutModal{{ $payout->id }}" tabindex="-1" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title">Complete Payout #{{ $payout->id }}</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <form action="{{ route('admin.affiliate.payouts.complete', $payout) }}" method="POST">
                                                @csrf
                                                <div class="modal-body">
                                                    <p>Requested Amount: <strong>${{ number_format($payout->amount, 2) }}</strong></p>
                                                    <div class="mb-3">
                                                        <label class="form-label">Actual Amount Paid ($)</label>
                                                        <input type="number" step="0.01" name="paid_amount" class="form-control" value="{{ $payout->amount }}" max="{{ $payout->amount }}" min="0" required>
                                                        <div class="form-text">If you pay less than requested, the difference returns to the affiliate's available balance.</div>
                                                    </div>
                                                </div>
