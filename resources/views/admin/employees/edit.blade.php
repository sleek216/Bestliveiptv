...
@section('content')
    <div class="mb-4 d-flex flex-wrap justify-content-between align-items-center gap-3">
        <div>
            <h1 class="page-title">Edit Employee: {{ $employee->name }}</h1>
            <p class="text-muted mb-0">Update employee details, credentials, and granted module permissions</p>
        </div>
        <div>
            ...
            <div class="col-lg-4">
                <div class="card mb-4">
                    <div class="card-header bg-white py-3">
                        <i class="bi bi-person-badge me-2 text-primary"></i>Employee Profile & Access
                    </div>
                    <div class="card-body">
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <small class="text-muted">Staff login username/email</small>
                            @error('email')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('designation') is-invalid @enderror" id="designation" name="designation" value="{{ old('designation', $employee->designation) }}" placeholder="e.g. Support Specialist">
                            @error('designation')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="p-3 bg-light rounded-3 mb-3">
                            <h6 class="fw-bold mb-1"><i class="bi bi-shield-lock me-1"></i>Change Password</h6>
                            <small class="text-muted d-block mb-3">Leave blank if you do not wish to change the existing password.</small>
                            
                            <div class="mb-3">
                                <label for="password" class="form-label">New Password</label>
                                <input type="password" class="form-control @error('password') is-invalid @enderror" id="password" name="password" minlength="8" placeholder="Leave blank to keep current">
                                @error('password')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-2">
                                ...
                                <input type="password" class="form-control" id="password_confirmation" name="password_confirmation" minlength="8" placeholder="Repeat new password">
                            </div>
                        </div>

                        <div class="form-check form-switch mt-4 mb-2">
                            <input class="form-check-input" type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $employee->isActive() ? '1' : '0') == '1' ? 'checked' : '' }}>
                            <label class="form-check-label fw-semibold" for="is_active">Account Active</label>
                            <small class="d-block text-muted">When switched off, this employee will be blocked from logging in.</small>
                        </div>
                    </div>
                </div>
                ...
                <div class="card mb-4">
                    <div class="card-header bg-white py-3 d-flex flex-wrap justify-content-between align-items-center gap-2">
                        <div>
                            ...
                            <small class="text-muted d-block">Adjust which sections and actions this employee is allowed to access</small>
                        </div>
                    ...
                    <div class="card-body p-4">
                        @php
                            $currentPerms = old('permissions', $assignedPermissions);
                        @endphp
                                        ...
                                        <div class="col-md-6">
                                            <div class="p-3 border rounded-3 h-100 perm-item-card transition-all" style="background: #f8fafc;">
                                                <div class="form-check">
                                                    <input class="form-check-input perm-checkbox group-{{ Str::slug($groupName) }}" type="checkbox" name="permissions[]" value="{{ $permKey }}" id="perm_{{ $permKey }}" {{ in_array($permKey, $currentPerms) ? 'checked' : '' }}>
